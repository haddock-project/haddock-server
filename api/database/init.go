...
func initTables() error {
	...
	//init the user table
	_, err = db.Exec("CREATE TABLE IF NOT EXISTS `users` (`user_name` varchar(255) NOT NULL UNIQUE PRIMARY KEY,`user_password` varchar(255) NOT NULL, `user_icon` BLOB DEFAULT '', `user_permissions` INT DEFAULT 0, `password_reset` BOOLEAN DEFAULT 0 CHECK (password_reset IN (0, 1)));")
	if err != nil {
		return err
	}
